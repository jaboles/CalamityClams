...
public class GameManager
{
	...
	private static final boolean DEBUG = false;	// Toggles some debugging code.
	public static final boolean TESTER = false;	// Toggles the automatic tester.
	...
	public synchronized int clamClicked(int x, int y, int what, int id)
	{
		...
		/* FINE GRAIN LOCK
		Object dummy = new Object(); // dummy object to lock if the grid cell we want to lock is out of range

		int points = 0;

		//top 6 sqaures
		synchronized(((y - 2 < 0 || x - 2 < 0) ? dummy: grid[x-2][y-2]))
		{
			synchronized ((y-2<0 ? dummy : grid[x][y-2]))
			{
				synchronized(((y-2<0 || x+2>XSIZE-1) ? dummy : grid[x+2][y-2]))
				{
					synchronized((y-1<0 || x-1<0 ? dummy : grid[x-1][y-1]))
					{
						synchronized ((y-1<0 ? dummy : grid[x][y-1]))
						{
							synchronized((y-1<0 || x+1> XSIZE-1 ? dummy : grid[x+1][y-1]))
							{
								//5 in the middle row
								synchronized ((x-2<0 ? dummy : grid[x-2][y]))
								{
									synchronized ((x-1<0 ? dummy : grid[x-1][y]))
									{
										synchronized (grid[x][y])
										{
											synchronized ((x+1>XSIZE-1 ? dummy : grid[x+1][y]))
											{
												synchronized ((x+2>XSIZE-1 ? dummy : grid[x+2][y]))
												{
													// bottom 6 squares
													synchronized((y+1>YSIZE-1 || x-1<0 ? dummy : grid[x-1][y+1]))
													{
														synchronized ((y+1>YSIZE-1 ? dummy : grid[x][y+1]))
														{
															synchronized((y+1>YSIZE-1 || x+1>XSIZE-1 ? dummy : grid[x+1][y+1]))
															{
																synchronized((y+1>YSIZE-1 || x-2<0 ? dummy : grid[x-2][y+1]))
																{
																	synchronized((y+2>YSIZE-1 ? dummy : grid[x][y+2]))
																	{
																		synchronized((y+2>YSIZE-1 || x+2>XSIZE-1 ? dummy : grid[x+2][y+2]))
																		{
																			//check and see if they win a point.
																			if(grid[x][y].intValue() != BLANK)
																			{
																				//BLANK = Empty Space
																				return -1;
