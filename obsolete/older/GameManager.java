...
public class GameManager
{
	...
	private int checkClamPoints(int x, int y)
	{
		...
		System.out.println(points+" points scored.");
		return points;
	}
	...
	public synchronized void updateClients(int x, int y, int what, int id)
	{
		updating = true;
		for (int i = 0; i < clients.size(); i++)
		{
			...
			if (sni.getPlayerId() != id) sni.placeObject(x, y, what);
		}
		updating = false;
		notifyAll();
	}
	...
	public synchronized void updateScore() {
		...
		int[] scores = new int[clients.size()];
		ServerNetworkInterface sni;
		for (int i = 0; i < clients.size(); i++) {
			sni = (ServerNetworkInterface)clients.get(i);
			names[i] = sni.getPlayerName();
			scores[i] = sni.getPlayerScore();
		}
		for (int i = 0; i < clients.size(); i++) {
			sni = (ServerNetworkInterface)clients.get(i);
			sni.updateScore(names,scores);
		}
		updating = false;
		notifyAll();
	}
